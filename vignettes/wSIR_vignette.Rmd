...
wSIR is an extension of the supervised dimension technique of Sliced Inverse Regression (SIR). 

To understand the innovation of wSIR, we must first go over SIR. SIR is an existing supervised dimension reduction method which works by grouping together the observations with similar values for the response. For spatial transcriptomics data, this means grouping all the cells into a certain number of tiles based on their spatial position. For example, if we use 4 tiles, then the cells in the top right quadrant go to one group, those in the top left to another, and so on. Each of those groups is then summarised by averaging the expresssion level of all cells in each group for each of the genes. From there, PCA is performed (not exactly but pretty much) on the resulting matrix of tile-gene means, then returning the SIR directions (PC directions) and SIR scores (PCs).

The motivation behind wSIR is that SIR only uses each cell's spatial position when we are separating the cells into the given number of groups/tiles. Once those groups are created, we lose the fact that some groups may be more spatially related (if they come from adjacent tiles) than other groups (if they come from opposite sides of the tissue). wSIR uses a weight matrix to incorporate the spatial correlation between all pairs of cells in the SIR algorithm. This matrix has dimension H*H, where H is the number of tiles, and the (i,j)th entry represents the distance between tiles i and j. 
...
Note: there is a BioConductor limit on the size of data that can be used for examples and vignettes. To stay under that, we randomly sample half of the cells from each of the three samples.
...
#set.seed(2024)
#keep1 = sample(c(TRUE, FALSE), nrow(sample1_exprs), replace = TRUE)
#keep2 = sample(c(TRUE, FALSE), nrow(sample2_exprs), replace = TRUE)
#keep3 = sample(c(TRUE, FALSE), nrow(sample3_exprs), replace = TRUE)

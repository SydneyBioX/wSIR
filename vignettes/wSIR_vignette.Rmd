...
#seqfish_data_sample1 <- LohoffSeqFISHData(samples = c(1,2)) # Note the samples titled 1 and two are actually both from sample1. They are referring to the two z slices for sample 1
...
This low-dimensional gene expression matrix can then be used for downstream tasks which would benefit from a low-dimensional embedding of sample 3's gene expression matrix that contains information about each cell's location. Examples for downstream use include 

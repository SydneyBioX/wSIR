...
#'
#' @description
#' wSIR function for specified parameters alpha, slices. To be used internally
#' in wSIROptimisation and wSIR functions, each for
#' specific values of alpha and slices.
#'
#' @param X matrix containing normalised gene expression data including n cells
...
#' @param group a factor indicating group level information for cells within
#' and across samples (e.g. cell type).
...
#' example, slices = 4 creates 4 slices along
#' each spatial axis, yielding 4^2 = 16 tiles. Default is 8, suggested
#' minimum of 3. Suggest to tune this parameter.
#' @param alpha integer to specify desired strength of spatial correlation.
#' Suggest to tune this parameter on testing
#' dataset among e.g values c(0,2,4,8). alpha = 0 gives SIR implementation.
#' Larger values give stronger spatial correlations.
#' @param ... arguments passed to sirCategorical
#'
#' @return wSIR object which is a list containing 5 (named) positions. 1)
...
#' 3) estd integer stating how many dimensions in the computed low-dimensional
#' space are needed to account for varThreshold
...
#' such that e.g the first j directions are included if the sum of the first
#' j evalues equals 0.95% of the sum of all evalues.
#'
